# Application Docs

This folder documents the implemented application, not the original requirement set.

Use these files as the current source of truth for:
- what the app does
- how the runtime flow works
- how operators refresh and verify the cache-backed demo

Files:
- `01-overview.md`: product shape, app surfaces, and user-facing behavior
- `02-runtime-flow.md`: request flow, cache flow, and summary generation path
- `03-operations.md`: setup, refresh, verification, and demo-day workflow
